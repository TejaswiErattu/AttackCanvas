/**
 * Saved OSV responses, for testing the scanner with no network.
 *
 * Captured live from https://api.osv.dev on 2026-09-20, unedited, by asking about
 * lodash@4.17.15, left-pad@1.3.0 and minimist@1.2.0 and then fetching every id the
 * batch returned. They are here as captured rather than tidied because the untidy parts
 * are the point:
 *
 *   - lodash@4.17.15 returns SIX advisories that are really four issues. GHSA-35jh and
 *     GHSA-r5fr list each other as aliases, as do GHSA-f23m and GHSA-xxjr;
 *   - GHSA-p6mc lists eight affected packages: lodash, lodash-es, lodash.pick, ... and a
 *     RubyGems gem, each with its own fix (lodash 4.17.19, lodash-es 4.17.20);
 *   - minimist has two ranges per advisory, one 0.x and one 1.x, so the first fix LISTED
 *     (0.2.1) is below the installed 1.2.0 and would be wrong advice; the first
 *     APPLICABLE fixed version is 1.2.3;
 *   - left-pad has no vulnerabilities, and its result is {} with no "vulns" key at all.
 */

/** The three packages the batch below was asked about, in order. */
export const CAPTURED_QUERIES = [
  { name: "lodash", version: "4.17.15" },
  { name: "left-pad", version: "1.3.0" },
  { name: "minimist", version: "1.2.0" },
] as const;

/** POST /v1/querybatch for CAPTURED_QUERIES. */
export const BATCH_RESPONSE: unknown = {
  results: [
    {
      vulns: [
        {
...
/** GET /v1/vulns/{id}, by id. */
export const VULNS: Record<string, unknown> = {
  "GHSA-29mw-wpgm-hmr9": {
    id: "GHSA-29mw-wpgm-hmr9",
    ...
    affected: [
      {
        ...
        database_specific: {
          source:
            "https://github.com/github/advisory-database/blob/main/advisories/github-reviewed/2022/03/GHSA-xvch-5gv4-984h/GHSA-xvch-5gv4-984h.json",
        },
      },
      {
        package: {
